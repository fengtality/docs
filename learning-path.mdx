...
|-------|------|--------------|------|
| **Beginner (Local)** | Get Condor running on your own machine with no Telegram account, and build one Trading Agent from the browser | [Installing → Local](/getting-started/installing#local) → [Web Dashboard](/getting-started/web-dashboard) → [Credentials](/getting-started/credentials) → [First Agent](/getting-started/first-agent) | ~30–45 min |
| **Beginner (Local + Telegram alerts)** | The same, with a bot attached so agents can reach your phone with alerts and approval requests | [Installing → Local + Telegram alerts](/getting-started/installing#local--telegram-alerts) → [Telegram](/getting-started/telegram) → [Credentials](/getting-started/credentials) → [First Agent](/getting-started/first-agent) | ~30–45 min |
...
`ADMIN_USER_ID` and carry across. Moving to a server later is the same install
on a different host.
