...
- **`/update`** — check for updates and restart Condor. Reports both Condor and Hummingbot API, and can update either or both. There is no dashboard equivalent: this one is Telegram-only, so a Local install updates by hand — see [Keeping Condor updated](/getting-started/installing#keeping-condor-updated).
