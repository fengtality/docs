...
In Telegram, **`/servers`** manages the same list.
...
Multiple **accounts** (`/accounts/add-account`) keep credential sets apart — paper vs. live, one strategy per account, or per-teammate keys. In Telegram, **`/keys`** manages credentials and **`/gateway`** the wallets.
