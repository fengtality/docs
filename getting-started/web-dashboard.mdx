...
<Tabs>
  <Tab title="Local mode">
    ...
    You are already signed in as `ADMIN_USER_ID`. Local mode binds `127.0.0.1`, so the dashboard is reachable from that machine only — see [Choosing Telegram or Local mode](/getting-started/installing#choosing-telegram-or-local-mode) before changing that.
  </Tab>
