...
- **[Tailscale](#secure-remote-access-with-tailscale) is not optional here.** Declining it publishes port 8000 on the VPS's public IP, and Docker's published ports are not blocked by `ufw`. See [Do I Need Tailscale?](#do-i-need-tailscale) for why.
...
| `make doctor` | `hummingbot-api` dir | The same for the API host, including whether port 8000 is on a public interface |
