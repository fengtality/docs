...
<Tip>
**New to Condor?** Head to the [Learning Path](/learning-path) to find the setup and reading order that matches your experience level and use case—whether that's letting AI trade for you, running an always-on bot, or deploying to production.
</Tip>
...
<CardGroup cols={2}>
  <Card title="Learning Path" icon="map" href="/learning-path">
    Find your route by experience level and use case
  </Card>
  <Card title="Motivation" icon="lightbulb" href="/motivation">
    Understand why Condor is architected this way
  </Card>
  <Card title="Installation" icon="download" href="/getting-started/installing">
...
These docs are published in a machine-readable form, so you can hand them to an LLM rather than paraphrasing them at it:

| File | Contents |
|---|---|
| [`/llms.txt`](https://condor.hummingbot.org/llms.txt) | An index of every page with its description — small enough to paste whole |
| [`/llms-full.txt`](https://condor.hummingbot.org/llms-full.txt) | The full text of every page in one file |

Any page also serves as Markdown by appending `.md` to its URL — for example [`/getting-started/installing.md`](https://condor.hummingbot.org/getting-started/installing.md).

<Tip>
  Condor's own agents already read the parts of this that matter to them. This is for **your** assistant — pointing Claude, Codex or Cursor at `/llms.txt` is usually enough for it to answer setup questions against the current docs instead of its training data.
</Tip>
