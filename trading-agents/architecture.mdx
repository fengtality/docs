...
**Configs** are agent-suggestible parameters that control how the agent operates. The agent can suggest changes based on learnings, but the user must approve.
...
**Pre-tick**: Blocks the entire tick if daily loss or drawdown limits exceeded.

**Per-executor**: Blocks creation if executor count, order size, or exposure limits would be violated.

If any limit is exceeded, the agent receives a clear error message explaining which limit was hit.
