...
```bash
curl -u YOUR_USERNAME:YOUR_PASSWORD -X POST http://localhost:8000/gateway/clmm/positions_owned \
...
```bash
curl -u YOUR_USERNAME:YOUR_PASSWORD -X POST http://localhost:8000/gateway/clmm/remove \
  -H "Content-Type: application/json" \
  -d '{
    "connector": "orca",
    "network": "solana-mainnet-beta",
...
```bash
curl -u YOUR_USERNAME:YOUR_PASSWORD -X POST http://localhost:8000/gateway/clmm/collect-fees \
  -H "Content-Type: application/json" \
  -d '{
    "connector": "orca",
    "network": "solana-mainnet-beta",
...
```bash
curl -u YOUR_USERNAME:YOUR_PASSWORD "http://localhost:8000/gateway/clmm/pools?connector=orca&search_term=SOL"
```
